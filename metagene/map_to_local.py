...
def map_to_local(
    ...
    ref_with_cumsum = ref_sorted_for_cumsum.group_by(ref_id_col, maintain_order=True).map_groups(
        calculate_cumsum_positions
    )
    ...
    query_strands = query_indexed["Strand"].to_numpy()
    ...
    ref_strands = ref_indexed["Strand"].to_numpy()
    ...
    result = (
        overlaps_df
        .join(query_indexed, on="_query_idx")
        .join(ref_indexed, on="_ref_idx", suffix="_ref")
    )
    ...
    result = result.with_columns([
        pl.max_horizontal(pl.col("Start"), pl.col("Start_ref")).alias("_intersect_start"),
    ...
    result = result.with_columns([
        ...
        .otherwise(
            pl.col("_intersect_start") - pl.col("Start_ref") + pl.col("_cumsum_start")
        )
        .alias("_local_start"),
    ...
    ref_id_col_name = ref_id_col if ref_id_col not in ["Chromosome", "Start", "End", "Strand"] else ref_id_col + "_ref"
    ...
    for col in original_query_cols:
        if col not in ["Chromosome", "Start", "End", "Strand"] and col in result.columns:
