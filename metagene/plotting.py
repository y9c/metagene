...
def plot_profile(
    ...
    ax.text(line_5utr_cds/2, ax.get_ylim()[1]*0.9, "5'UTR", 
            ha='center', va='center', fontweight='bold')
    ax.text((line_5utr_cds + line_cds_3utr)/2, ax.get_ylim()[1]*0.9, "CDS", 
            ha='center', va='center', fontweight='bold')
    ax.text((line_cds_3utr + 1)/2, ax.get_ylim()[1]*0.9, "3'UTR", 
            ha='center', va='center', fontweight='bold')
...
def plot_binned_statistics(
    ...
    mean_cols = [col for col in df_score.columns if col.endswith('_mean') or col == 'mean']
...
def simple_metagene_plot(df: pl.DataFrame, gene_splits: tuple, output_path: str):
    """
    Create the metagene profile plot using matplotlib.
    
    Args:
        df: DataFrame with normalized positions
    ...
    feature_positions = df["feature_pos"].drop_nulls().to_numpy()
    ax.hist(
        feature_positions,
        bins=100,
        color="blue",
    ...
    # Add region labels
    ax.text(gene_splits[0]/2, ax.get_ylim()[1]*0.9, "5'UTR", ha="center", va="top", fontsize=12, fontweight="bold")
    ax.text(gene_splits[0] + gene_splits[1]/2, ax.get_ylim()[1]*0.9, "CDS", ha="center", va="top", fontsize=12, fontweight="bold")
    ax.text(gene_splits[0] + gene_splits[1] + gene_splits[2]/2, ax.get_ylim()[1]*0.9, "3'UTR", ha="center", va="top", fontsize=12, fontweight="bold")
    
    # Set labels and title
    ax.set_xlabel("Normalized Gene Position", fontsize=12)
    ax.set_ylabel("Density", fontsize=12)
    ax.set_title("Metagene Profile - Package Function Demo", fontsize=14, fontweight="bold")
    ax.legend()
    ax.grid(True, alpha=0.3)
